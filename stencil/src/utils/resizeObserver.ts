...
export const resizeObserver = (el) => {
  for (let att, i = 0, atts = el.attributes, n = atts.length; i < n; i++) {
    ...
    if (
      !startsWith(att.nodeName, 'bp-mobile') &&
      startsWith(att.nodeName, 'bp-')
    ) {
      ...
      for (
        var att1, i1 = 0, atts1 = el.attributes, n1 = atts1.length;
        i1 < n1;
        i1++
      ) {
        att1 = atts1[i1];
        if (
          !startsWith(att1.nodeName, 'bp-mobile') &&
          startsWith(att1.nodeName, 'bp-')
        ) {
