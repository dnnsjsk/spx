...
export class SpxSectionButton {
    ...
    @Prop({ reflect: true }) transitionTimingFunction: string = c.transitionTimingFunction
    ...
    private applyColors (type) {
      ...
      const style = this.type === 'primary' ? 'var(--spx-color-primary-A700)' : 'var(--spx-color-primary-045)'

      /** Get the hue. */

      const styleHue = style.substring(style.lastIndexOf('-')).slice(1, -1).replace(/^0+/, '')
      ...
      if (type === 'background') {
        return this.type === 'translucent'
          ? (color === '#ffffff' ? 'rgba(0,0,0,0.2)' : 'rgba(255,255,255,0.2)')
          : style
      }
      ...
      if (type === 'background-hover') {
        return this.type === 'primary' ? palette[state.colorPrimary]['600']['color']
          : this.type === 'translucent' ? (color === '#ffffff' ? 'rgba(0,0,0,0.3)' : 'rgba(255,255,255,0.3)')
