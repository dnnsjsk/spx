...
@Component({
  tag: 'spx-control-number',
  styleUrl: 'spx-control-number.scss',
  shadow: true,
})
export class SpxControlNumber {
  ...
  @Element() el: HTMLSpxControlNumberElement;
  ...
  componentDidLoad() {
    if (this.slider) {
      ...
      this.sliderEl.on('update', (values) => {
        this.value = values[0];
        (this.input as HTMLInputElement).value = values[0];
      });
    ...
    return (
      <Host>
        <div class="label">
          ...
          {this.slider && <spx-control-label label={this.value} mb={true} />}
        </div>
        {this.slider && (
          <div ref={(el) => (this.container = el as HTMLElement)} />
        )}
        <spx-control-input
          style={{ display: this.slider ? 'none' : 'block' }}
          data={this.data}
          handleInput={this.onInput}
          max={!this.slider && this.max}
          min={!this.slider && this.min}
          ref={(el) => (this.input = el as HTMLElement)}
          step={!this.slider && this.step}
          type={this.slider ? 'text' : 'number'}
          value={this.value}
        />
      </Host>
    );
