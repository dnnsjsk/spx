...
function spxLicenseMenu() {
	add_submenu_page( 'tools.php', 'spx', 'spx', 'manage_options', SPX_LICENSE_PAGE, 'spxLicensePage' );
}

add_action('admin_menu', 'spxLicenseMenu', 11);
...
function spxActivateLicense() {
	...
	if( isset( $_POST['spx_license_activate'] ) ) {
		...
		if ( ! empty( $message ) ) {
			$base_url = admin_url( 'tools.php?page=' . SPX_LICENSE_PAGE );
		...
		wp_redirect( admin_url( 'tools.php?page=' . SPX_LICENSE_PAGE ) );
		exit();
	}
}
...
function spxDeactivateLicense() {
	...
	if( isset( $_POST['spx_license_deactivate'] ) ) {
		...
		// retrieve the license from the database
		$license = trim( get_option( 'spx_license_key' ) );
        $license = $_POST['spx_license_key'] && strlen($_POST['spx_license_key']) > 8 ? sanitize_text_field($_POST['spx_license_key']) : $license;
