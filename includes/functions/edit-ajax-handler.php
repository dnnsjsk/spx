...
function spxEditButtonAjaxHandlerGetString($value, $type): string
{
    return trim(
        wp_kses(
            htmlspecialchars(
                str_replace($_POST["nonce"] . $type, "", $value),
                ENT_QUOTES,
                "UTF-8"
            ),
            false
    ...
    if (wp_verify_nonce($_POST["nonce"], "ajax-nonce")) {
        ...
        foreach ($_POST as $key => $value) {
            if (strpos($value, $_POST["nonce"] . "acf") !== false) {
                update_field(
                    $key,
                    spxEditButtonAjaxHandlerGetString($value, "acf"),
                    $post_id
                );
            }
            ...
            if (strpos($value, $_POST["nonce"] . "option") !== false) {
                update_option(
                    $key,
                    spxEditButtonAjaxHandlerGetString($value, "option")
                );
            }
            ...
            if (strpos($value, $_POST["nonce"] . "parentacf") !== false) {
                ...
                update_sub_field(
                    $array,
                    spxEditButtonAjaxHandlerGetString($value, "parentacf"),
                    $post_id
                );
            }
        }
